...
namespace UnSealer.Protections.Mutations.Purifier
{
    public class NopPurifier : ProtectionPhase
    {
        ...
        public override void Execute(Context context, IEnumerable<MetadataMember> targets)
        {
            ...
            {
                AsmResolver.DotNet.Code.Cil.CilInstructionCollection IL = Method.CilMethodBody.Instructions;
                IEnumerable<CilInstruction> Branches = Utils.GetBranches(Method.CilMethodBody);
                foreach (CilInstruction Instr in IL.Where(x => x.IsCode(CilCode.Nop) && !Branches.Contains(x)).ToArray())
                {
                    IL.Remove(Instr);
                }
                IL.CalculateOffsets();
